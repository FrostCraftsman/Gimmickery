...
public class Gimmickery250Packet {
	...
	public static Gimmickery250Packet parse(byte[] bytes) throws IOException
	{
	...
	private void writeData(DataOutputStream data) throws IOException
	{
	    ...
	    else
	    {
	     ...
	     if (this.dataBoolean != null){
	    	 //dataBoolean不为null则写入其长度
	    	 data.writeByte(this.dataBoolean.length);
	     }else{
	    	 //dataBoolean为null则写入0
	    	 data.writeByte(0);
	     }
	    //写入dataString数组的所有内容
	    if (this.dataString != null)
	    ...
	    if (this.dataBoolean != null){
	    	for (boolean bool: this.dataBoolean){
	...
	private void readData(DataInputStream data) throws IOException
	{
	    ...
	    //读取boolean数组长度
	    byte nBool = data.readByte();
	    if ((nString > 128) || (nInt > 128) || (nByte > 128) ||(nBool>128)|| (nString < 0) || (nInt < 0) || (nByte < 0)||(nBool<0))
	    {
	    ...
	    if (nBool==0){
	    	...
	    	this.dataBoolean=new boolean[nBool];
	    	for (int k=0;k<nBool;k++){
	    		this.dataBoolean[k]=data.readBoolean();
	    	}
	    }
	}
